      ...
      <nav class="hidden md:flex flex-col h-full py-lg px-md gap-md bg-surface-container-low text-primary border-r border-outline-variant w-64 fixed left-0 top-0 transition-all duration-200 ease-in-out">
        ...
        <div class="flex flex-col gap-sm flex-grow">
          ...
          <a class="flex items-center gap-md px-md py-sm rounded-lg bg-primary-container text-on-primary-container font-bold transition-all cursor-pointer">
            ...
            <span class="font-label text-label">Incidents</span>
          </a>
          ...
          <a class="flex items-center gap-md px-md py-sm rounded-lg text-on-surface-variant hover:bg-surface-variant transition-all cursor-pointer">
            ...
            <span class="font-label text-label">Reports</span>
          </a>
        </div>
        ...
        <div class="flex flex-col gap-sm border-t border-outline-variant pt-md">
          <a class="flex items-center gap-md px-md py-sm rounded-lg text-on-surface-variant hover:bg-surface-variant transition-all cursor-pointer">
            ...
            <span class="font-label text-label">Settings</span>
          </a>
          <a (click)="logout()" class="flex items-center gap-md px-md py-sm rounded-lg text-on-surface-variant hover:bg-surface-variant transition-all cursor-pointer">
            ...
            <span class="font-label text-label">Logout</span>
          </a>
        </div>
      </nav>
        ...
        <header class="sticky top-0 z-50 flex items-center justify-between px-lg py-sm w-full bg-surface border-b border-outline-variant">
          <div class="flex items-center gap-md">
            ...
            <div class="hidden md:flex items-center bg-surface-container-low rounded-lg px-md py-xs border border-outline-variant focus-within:border-primary-container focus-within:ring-1 focus-within:ring-primary-container transition-all">
              ...
              <input class="bg-transparent border-none focus:ring-0 text-body font-body text-on-background placeholder-on-surface-variant outline-none w-64" placeholder="Search..." type="text"/>
            </div>
          </div>
        ...
        <main class="flex-1 overflow-y-auto p-md md:p-xl bg-background">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-lg">
            ...
            <div class="lg:col-span-2 flex flex-col gap-lg">
              ...
              <div class="bg-surface-bright rounded-xl border border-outline-variant p-lg">
                <div class="flex items-center gap-sm mb-md">
                  <span [ngClass]="getPriorityBadgeClass(incident()?.priority)" class="font-label text-label px-sm py-xs rounded-full inline-flex items-center gap-xs">
                    ...
                    {{ incident()?.priority || 'N/A' }} Priority
                  </span>
              ...
              <!-- Evidence Gallery -->
              <div class="bg-surface-bright rounded-xl border border-outline-variant p-lg">
                <h2 class="font-heading-md text-heading-md text-on-background mb-md">Evidence Gallery</h2>
              ...
              <!-- Timeline/History -->
              <div class="bg-surface-bright rounded-xl border border-outline-variant p-lg">
                <h2 class="font-heading-md text-heading-md text-on-background mb-md">Activity Timeline</h2>
                ...
                <div class="mt-md border-t border-outline-variant pt-md">
                  <label class="font-label text-label text-on-background mb-xs block">Add Note</label>
                  <textarea class="w-full bg-surface rounded-lg border border-outline-variant p-sm font-body text-body text-on-background focus:ring-1 focus:ring-primary-container focus:border-primary-container outline-none" placeholder="Type an internal note or update..." rows="3" [(ngModel)]="newNote" name="note"></textarea>
                  <div class="flex justify-end mt-sm">
                    <button (click)="addNote()" class="px-md py-sm rounded-lg bg-surface-container-low text-primary-container font-label text-label hover:bg-surface-variant transition-colors border border-outline-variant">Post Note</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="flex flex-col gap-lg">
              <!-- Metadata Card -->
              <div class="bg-surface-bright rounded-xl border border-outline-variant p-lg">
                <h3 class="font-heading-md text-heading-md text-on-background mb-md border-b border-outline-variant pb-sm">Details</h3>
                <div class="space-y-md">
                  <div>
                    <span class="font-caption text-caption text-on-surface-variant block mb-xs">Status</span>
                  ...
                  <div>
                    <span class="font-caption text-caption text-on-surface-variant block mb-xs">Created At</span>
                    <span class="font-body text-body text-on-background">{{ incident()?.createdAt || incident()?.dateReported }}</span>
                  </div>
                  <div>
                    <span class="font-caption text-caption text-on-surface-variant block mb-xs">Category</span>
              ...
              <div class="bg-surface-bright rounded-xl border border-outline-variant p-lg">
                <h3 class="font-heading-md text-heading-md text-on-background mb-md border-b border-outline-variant pb-sm">Reporter</h3>
                <div class="flex items-center gap-md">
                  ...
                  <div>
                    <div class="font-label text-label text-on-background">{{ incident()?.reporterName || 'Unknown' }}</div>
                    <div class="font-caption text-caption text-on-surface-variant">{{ incident()?.reporterRole || 'Citizen' }}</div>
                    <a class="font-caption text-caption text-primary hover:underline mt-xs inline-block cursor-pointer">View Profile</a>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-surface-bright rounded-xl border border-outline-variant p-lg flex flex-col h-64">
                <h3 class="font-heading-md text-heading-md text-on-background mb-md">Location</h3>
    ...
    const timeline: TimelineItem[] = [
      {
        ...
        author: incident.reporterName || 'Reporter',
        timestamp: '1 day ago'
      },
      {
  ...
  addNote() {
    if (this.newNote.trim()) {
      const newTimelineItem: TimelineItem = {
        action: 'Note added',
        author: this.authService.user()?.firstName + ' ' + this.authService.user()?.lastName || 'Current User',
        timestamp: 'just now',
